...
export default function WorkScheduleComponent() {
    ...
    const items = {
        "2025-03-17": [{ name: "Meeting with Team", time: "10:00 AM" }],
        "2025-03-18": [{ name: "Doctor Appointment", time: "02:00 PM" }],
        "2025-03-19": [{ name: "Project Deadline", time: "11:59 PM" }],
    };

    useEffect(() => {
        setSchedule(items); // Load initial items
    }, []);

    function clickNextWeek() {
        setSelectedDate(moment(selectedDate).add(7, "days").startOf("isoWeek").format("YYYY-MM-DD"));
    }

    function clickPreviousWeek() {
        setSelectedDate(moment(selectedDate).subtract(7, "days").startOf("isoWeek").format("YYYY-MM-DD"));
    }
            ...
            <View style={{flex: 1}}>
                <Agenda
                    ...
                    onDayPress={(day: object) => {
                        setSelectedDate(moment(day.dateString).startOf("isoWeek").format("YYYY-MM-DD"));
                    }}
                    renderEmptyData={() => (
...
const styles = StyleSheet.create({
    ...
    item: { backgroundColor: "#f9c2ff", padding: 15, borderRadius: 10, margin: 10 },
